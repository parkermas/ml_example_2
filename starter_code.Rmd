...
logistic_reg() %>%
  set_engine("glm") %>%
  fit(formula = dvote ~ gender + party + state + office, data = clean) %>%
...
clean_split <- initial_split(clean, prop = 1/100)
train_data <- training(clean_split)
test_data <- testing(clean_split)

logistic_reg() %>%
  set_engine("glm") %>%
  fit(formula = dvote ~ gender + party + state + office, data = train_data) %>%
